...
class Tag extends Component {
  ...
  render(){
    return (
      <div className="tagContainer" onMouseDown={this.dragTo} onDoubleClick={this.toggleEditable} style={{left:this.props.left,top:this.props.top}}>
        {this.state.editable ? <input onClick={(e)=>{e.stopPropagation()}} onBlur={this.changeText}/> : <div onClick={this.toggleExitVisibility} className="container">{this.state.text}</div>}
        <div className="exit" onClick={this.remove} style={{display:this.state.exitVisible ? "none" : "block"}}>X</div>
      </div>
    )
  }
}
